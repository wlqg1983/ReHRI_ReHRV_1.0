...
#######################################################################################################################################################################
# 单线程处理配对
def single_process_pairs(all_pairs, df):
    processed_data = []
    for pair in all_pairs:
        processed_data.extend(process_pair(pair, df))
    return processed_data
...
################################################################################
    ...
    df['prefix'] = df['fragment_id'].str.extract(r'(RU\d+)')
    ...
    # 采用单线程，对每一配对的重复单元进行处理
    processed_data = single_process_pairs(all_pairs, df)
