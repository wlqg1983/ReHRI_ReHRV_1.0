...
def trim_IR_sequence_circle(genome, fields, output_dir, trim_len, id1, id2, rev_compl_IR): 
    ...
    if len(genome.seq) - len(rep1_sequence) >= trim_len:
        ...
        if not repeat1_rev:
            ab_record = SeqRecord(ab_sequence, id=f"IR_LR_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if repeat1_rev:
            ab_sequence = reverse_complement_dna(ab_sequence)
            ab_record = SeqRecord(ab_sequence, id=f"IR_LR_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"IR_LR_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"IR_LR_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
    else:
        ...
        if not repeat1_rev:
            ab_record = SeqRecord(ab_sequence, id=f"IR_LR_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if repeat1_rev:
            ab_sequence = reverse_complement_dna(ab_sequence)
            ab_record = SeqRecord(ab_sequence, id=f"IR_LR_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"IR_LR_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"IR_LR_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"IR_LR_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
...
################################################################################
    if len(genome.seq) - len(rep2_sequence) >= trim_len:
        ...
        if not repeat2_rev:
            cd_record = SeqRecord(cd_sequence, id=f"IR_UD_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if repeat2_rev:
            cd_sequence = reverse_complement_dna(cd_sequence)
            cd_record = SeqRecord(cd_sequence, id=f"IR_UD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"IR_UD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"IR_UD_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
    else:
        ...
        if not repeat2_rev:
            cd_record = SeqRecord(cd_sequence, id=f"IR_UD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record = SeqRecord(cd_sequence, id=f"IR_UD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"IR_UD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"IR_UD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"IR_UD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
...
def trim_DR_sequence_circle(genome, fields, output_dir, trim_len, id1, id2, rev_compl_DR):
    ...
    if len(genome.seq) - len(rep1_sequence) >= trim_len:
        ...
        ab_sequence = genome_reset[repeat1_new_start-trim_len-1 : repeat1_new_end + trim_len]
        if not repeat1_rev:
            ab_record = SeqRecord(ab_sequence, id=f"DR_LR_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if repeat1_rev:
            ab_sequence = reverse_complement_dna(ab_sequence)
            ab_record = SeqRecord(ab_sequence, id=f"DR_LR_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"DR_LR_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"DR_LR_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
    else:
        lasso_trim_len = len(genome.seq) - len(rep1_sequence)  #trim_len，计算截取序列的长度
        upstream_seq = genome.seq[:repeat1_start-1]
        downstream_seq = genome.seq[repeat1_end:]
        ...
        ab_sequence = downstream_seq + genome.seq + upstream_seq  # 假设read已经套圈基因组
        if not repeat1_rev:
            ab_record = SeqRecord(ab_sequence, id=f"DR_LR_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if repeat1_rev:
            ab_sequence = reverse_complement_dna(ab_sequence)
            ab_record = SeqRecord(ab_sequence, id=f"DR_LR_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"DR_LR_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat1_rev:
            ab_sequence_c = reverse_complement_dna(ab_sequence)
            ab_record_c = SeqRecord(ab_sequence_c, id=f"DR_LR_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ab_record_c, os.path.join(output_dir, f"DR_LR_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
...
################################################################################
    if len(genome.seq) - len(rep2_sequence) >= trim_len:
        ...
        cd_sequence = genome_reset[repeat2_new_start-trim_len-1 : repeat2_new_end+trim_len]
        if not repeat2_rev:
            cd_record = SeqRecord(cd_sequence, id=f"DR_UD_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if repeat2_rev:
            cd_sequence = reverse_complement_dna(cd_sequence)
            cd_record = SeqRecord(cd_sequence, id=f"DR_UD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"DR_UD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"DR_UD_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
    else:
        lasso_trim_len = len(genome.seq) - len(rep2_sequence)
        upstream_seq = genome.seq[:repeat2_start-1]
        downstream_seq = genome.seq[repeat2_end:]
        ...
        cd_sequence = downstream_seq + genome.seq + upstream_seq  # 假设read已经套圈基因组
        if not repeat2_rev:
            cd_record = SeqRecord(cd_sequence, id=f"DR_UD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if repeat2_rev:
            cd_sequence = reverse_complement_dna(cd_sequence)
            cd_record = SeqRecord(cd_sequence, id=f"DR_UD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"DR_UD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat2_rev:
            cd_sequence_c = reverse_complement_dna(cd_sequence)
            cd_record_c = SeqRecord(cd_sequence_c, id=f"DR_UD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(cd_record_c, os.path.join(output_dir, f"DR_UD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
...
def read_last_line(filename):
    with open(filename, 'rb') as file:  # Open in binary mode
        try:
            ...
            # Keep reading backwards until we hit a newline
            while file.read(1) != b'\n':
                file.seek(-2, os.SEEK_CUR)
        except OSError:
            # File is too small or only one line
            file.seek(0)
