...
def find_chromosome(start, end, cumulative_lengths):
    ...
    try:
        ...
        raise ValueError("start和end必须是整数或可以转换为整数的值")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="OGRecomFinder: A tool to check spanning reads for supporting subconfig of your organelle genome.")
...
################################################################################
        ...
        if not seqdepth_third:                # 当提供的不是三代数据的时候，但是又提供了三代数据的类型，则给出提示，将类型设置为 None
            if seqdepth_type:
                logging.warning(f"The third sequencing data type {TGS_type} was not required.")
                logging.warning("The 'TGS type' was reset to None.")
                time.sleep(3)
                seqdepth_type = ''
        ...
        if seqdepth_threads.strip() == '':
            ...
            # 如果不是空字符串，则尝试转换为整数
            try:
                seqdepth_threads = int(float(seqdepth_threads))
            except ValueError:
...
################################################################################
        ...
        if mode == "R" and count >1:
            if os.path.isfile(f"ROUSFinder_results_{project_id}/{rous_output_file_prefix}_{project_id}_rep_table.txt"):
                ...
                if len(df) == 0:
                    logging.warning(f"The DataFrame {file_path} may be empty.")
                else:
                    # 应用函数并分别更新列  
...
################################################################################
        ...
        if mode == 'C':
            ...
            manually_calibrate_reset = f"ROUSFinder_results_{project_id}/{prefix_manual}_{project_id}_calibration_table.txt"          ##更换表头之后，保存为calibration_table.txt
            ...
            # 以上 manually_calibrate_reset 对应的文件 adjusted_manual_calibrated_list_calibration_table.txt
            ...
            check_files = [
                ...
                f"ROUSFinder_results_{project_id}/{prefix_manual}_{project_id}_calibration_table.txt"        # 所以ROUSFinder_results中只应含有rep_5CT.tsv
                ]
            # 筛选出存在的文件
            ...
            if provided_data_type in ['NGS_single_end', 'TGS']:
                ...
                if filter_reads in ['Y','YES']:        ############################################## 开始过滤reads，提高后面比对的速度。
                    if file_format == 'FASTQ':     #not os.path.isfile(f"{fasta_prefix}.fasta"):        # 当输入文件为fastq格式，进行格式转换
                        ...
                        fasta_one = fastq_to_fasta(seq_data_value, fastq_prefix + ".fasta")
                        fastq2fasta = True
                    elif file_format == 'FASTA':
                        fasta_one = seq_data_value    # f"{fasta_prefix}.fasta"    # 为后面读取fasta设置路径
                    
                    fasta_prefix = os.path.basename(fasta_one).split('.')[0]         # 提取前缀, 此时的文件一定是fasta格式，与之前的一句重复
                    # 过滤reads
                    ...
                    #seq_data_value = fasta_one       # 统一进行格式转换
            ...
            if provided_data_type == 'NGS_pair_ends':
                ...
                if filter_reads in ['Y','YES']:        ############################################## 开始过滤 reads，提高后面比对的速度。
                    if file_format == 'FASTQ':     #not os.path.isfile(f"{fasta_prefix}.fasta"):    # 当输入文件为 fastq 格式，进行格式转换
                        ...
                        fasta_pair1 = fastq1    # f"{fasta_prefix}.fasta"    # 为后面读取fasta设置路径

                    fasta1_prefix = os.path.basename(fasta_pair1).split('.')[0]     # 提取前缀
                    # 过滤reads
                ...
                fastq2_prefix = os.path.basename(fastq2).split('.')[0]     # 提取前缀
                ...
                if filter_reads in ['Y','YES']:
                    if file_format == 'FASTQ':     #not os.path.isfile(f"{fasta_prefix}.fasta"):    # 当输入文件为fastq格式，进行格式转换
                        ...
                        fasta_pair2 = fastq2    # f"{fasta_prefix}.fasta"    # 为后面读取fasta设置路径
                    fasta2_prefix = os.path.basename(fasta_pair2).split('.')[0]     # 提取前缀
...
################################################################################
        if mode in ['A', 'C']:
            ...
            for fasta_file in subcon_fasta_files:    ##################### 这里开始处理次要构型 subconfiguration
                ...
                if os.path.basename(fasta_file) in processed_files:
                ...
                run_command(["python", os.path.join(dir_path, "count_spanning_reads.py"), "-i", spanning_read_file, "-o", file_path_sub]) 

                # 处理完一个trimmed seq后，将其添加到记录文件中
        ...
        if mode in ['A', 'C'] and redundant_intermediate_results == "D":  
            ...
            for trimmed_seq_supp_recomb_folder in trimmed_seq_supp_recomb_folders:  
                ...
                if len(parts) >= 4:  
                    ...
                    # 将提取的信息追加到文件中  
                    with open(output_filename, 'a') as f:  # 'a' 表示追加模式  
                        f.write(f"{third_column}\t{fourth_column}\n")  # 使用制表符分隔列，并添加换行符            
...
########################################################################################################################################################################
        ...
        if mode == "C" and count >1:
            ...
            if os.path.isfile(f"{manually_calibrate}"): 
                file_path = f"{manually_calibrate}"
                df = pd.read_csv(file_path, sep='\t')  
                df = df.dropna(how='all')    # 去掉空白行

                if len(df) == 0:
                    logging.warning(f"The DataFrame {file_path} may be empty.")
                    new_file_path = f"ROUSFinder_results_{project_id}/trace_Start_End_{file_name_manual}" 
                    shutil.copy(file_path, new_file_path)  
                else:                    
                    # 指定新的表头  
                    new_header = ['fragment_id', 'length', 'start', 'end', 'direction', 'chromosome'] 
                    ...
                    # 保存修改后的 DataFrame  
                    file_path = f"ROUSFinder_results_{project_id}/trace_Start_End_{file_name_manual}_{project_id}"    # 用户本身提供的就是伪基因组上的重复序列，这里是给追溯回每个repeat的真正起点和终点
                    df.to_csv(file_path, sep='\t', index=False)  
...
################################################################################
        ...
        if mode in ['A', 'C'] and count > 1:
            if os.path.isfile(f"{project_id}/{output_dir_prefix}_{project_id}/one_chain_without_sufficient_spanning_reads.tsv"):  
                ...
                # 检查文件大小，确定是否为空文件  
                if is_file_empty_or_invisible_chars(file_path): 
                    default_headers = ['fragment_id', 'length', 'start', 'end', 'direction', 'chromosome', 'plus_ratio(s/m)', 'minus_ratio(s/m)', 'combined_ratio', 'type', 'paired_id', 'paired_length', 'paired_start', 'paired_end', 'paired_direction', 'paired_chromosome', 'paired_plus_ratio(s/m)', 'paired_minus_ratio(s/m)', 'paired_combined_ratio']
                    # 文件为空，将默认表头写入文件  
                    with open(file_path, 'w', newline='', encoding='utf-8') as f:  
                        f.write('\t'.join(default_headers) + '\n') 
...
################################################################################
        ...
        logging.info("\nThe computational process of searching for repeats that can mediate genome recombination has been completed.!\n")


################################################################################
    elif  mode in ['A', 'R', 'C'] and refilter_mode:    # 主程序的运行逻辑
        logging.error("Please reset 'refilter_mode = N' in the [refilter_results] section. Two modes can't be run at the same time!\n")
        sys.exit(1)
...
################################################################################
    ...
    if mode == "N" and refilter_mode:
        ...
        #print("The operation needs to be performed in the directory where the pipline was run previously, and do NOT MODIFY the names of previously generated result files.\n")
        time.sleep(5)
        ...
        if project_id == refilter_id:
            logging.info(f"The 'refilter_id' in the [refilter_results] section cannot be the same as the 'project_id' in the [general] section.")
        ...
        check_spanning_length = config['refilter_results'].getint('spanning_read_flanking_repeat_length', 5)            # 读取新旧跨越重复单元的序列长度，用于重新过滤结果
        try:      #检查跨越重复序列的长度参数是否合法
            validate_positive_integer(check_spanning_length_old, 'check_spanning_length_old')
        except ValueError as e:
        ...
        if check_spanning_length_old == check_spanning_length:
            is_spanning_length_same = True                     # 创建逻辑变量，新旧跨越重复单元的序列长度是否相等。
        elif check_spanning_length_old < check_spanning_length:
            is_spanning_length_same = False
        else:
            logging.error("The value of 'spanning_read_flanking_repeat_length' in the [refilter_results] section should not be less than that in the [check_spanning_reads] section.")
            exit(1)
        ...
        if not inputfasta:
            logging.error("When 'refilter_mode' in the [refilter_results] section is 'y/yes', the 'inputfasta' in section [general] cannot be empty.")
            sys.exit(1)
        ...
        if os.path.isfile(f"{project_id}/custom_log_{project_id}.txt"):
            ...
            has_difference = compare_ini_contents(f"old_log_{project_id}.ini", args.config, keys_to_compare)
            if has_difference:
                logging.error(f"Previous '.ini' file {args.config} has been changed, except 'mode' and ['refilter_results'] section.")
...
################################################################################
        ...
        read_number_again = config['refilter_results'].getint('spanning_read_number', 5)
        ...
        read_number = config['check_spanning_reads'].getint('spanning_read_number', 1)
        if float(read_number_again) < float(read_number):
...
################################################################################
        ...
        # 重新筛选的原理
        # 基于以下两个中间结果文件，重新计算repeat结果，而不是真的从之前的最终结果里面再次筛选
        if not os.path.isfile(f"{project_id}/{final_output_dir_sub}"):
            logging.error(f"An indication error occurred! Previous results {project_id}/{final_output_dir_sub} may be changed.")
            sys.exit(1)
        if not os.path.isfile(f"{project_id}/{final_output_dir_main}"):
            logging.error(f"An indication error occurred! Previous results {project_id}/{final_output_dir_main} may be changed.")
            sys.exit(1)
        ...
        # 基于以下两个中间结果文件，重新计算repeat结果，而不是真的从之前的最终结果里面再次筛选
        if is_spanning_length_same and mode_old == "A":
            #print()
            logging.info("Since 'spanning_read_flanking_repeat_length' has not been reset, there is no need to re-filter the mapping results this time.\n")
            if count == 1:         # 当 spanning_length 相等时
        ...
        if is_spanning_length_same and mode_old == "C":
            #print()
            logging.info("Since 'spanning_read_flanking_repeat_length' has not been reset, there is no need to re-filter the mapping results this time.\n")
            if count == 1:         # 当 spanning_length 相等时
        ...
        # 当跨越重复单元的序列长度前后不一的时候，要重新进行过滤 sam 文件
        if not is_spanning_length_same:
            #print()
            logging.info("Since 'spanning_read_flanking_repeat_length' has been reset. It will re-filter the mapping results again.\n")
            time.sleep(5)
            ###################
            # 对次要构型进行重过滤
            map_results_folders = glob.glob(f"{project_id}/{extrsubcon_output_dir_prefix}_{project_id}/*_results")    # 找到存储所有结果的文件夹
            file_path_main = os.path.join(project_id, extrsubcon_output_dir_prefix + "_" + project_id, "refilter_subconfig_spanned_results.tsv")    # 创建存放结果的文件
            ...
            for map_results_folder in map_results_folders:       # 针对每一个存放结果的文件夹\
                ...
                if os.path.isdir(map_results_folder):
                    output_bam = os.path.join(map_results_folder, "spanning_reads_sorted.bam")     # 找出其中的 bam 文件
                    folder_refilter = f"{map_results_folder}_{refilter_id}"
                    if os.path.exists(folder_refilter):
                        # 如果路径是一个文件，删除该文件
                        if os.path.isfile(folder_refilter):
                            os.remove(folder_refilter)
                        elif os.path.isdir(folder_refilter):
                            # 如果路径是一个目录，可以选择删除该目录及其内容
                            shutil.rmtree(folder_refilter)
                    os.makedirs(folder_refilter)                    # 创建存放 再次过滤结果的文件夹 folder_refilter

                    output_sam = os.path.join(folder_refilter, "spanning_reads_sorted.sam")
                    # 将 BAM 转换为 SAM 并将结果输出到指定文件，将 之前的 bam 文件 转换为 sam 文件，并保存至 folder_refilter
                    with open(output_sam, 'w') as f:
                        command = ["samtools", "view", "-h", output_bam]
                        subprocess.run(command, stdout=f, check=True)

                    # 复制 .fasta 文件到 目标文件夹 folder_refilter
                    # 此时，folder_refilter 内存有 转格式来的 sam 文件和 序列文件 fasta 
                    for fasta_file in glob.glob(os.path.join(map_results_folder, "*.fasta")):
                        shutil.copy(fasta_file, folder_refilter)

                    fasta_files = glob.glob(os.path.join(folder_refilter, "*.fasta"))
                    if len(fasta_files) == 1:
                        fasta_file = fasta_files[0]
                    else:
                        raise FileNotFoundError("There should be exactly one '.fasta' file in the folder_refilter directory")
                    ...
                    run_command(["python", os.path.join(dir_path, "check_subconfig_spanning_reads.py"), "-s", output_sam, "-i", fasta_file, "-o", folder_refilter, "-cl", str(check_spanning_length), "-tl", str(extrsubcon_trimmed_length_reset), "-rn", str(read_number_again), "-mr", redundant_intermediate_results, "-a", seqdepth_alignment_software])

                    for fasta_file in glob.glob(os.path.join(map_results_folder, "*.fasta")):   # check_subconfig_spanning_reads.py 将其删除了
                        shutil.copy(fasta_file, folder_refilter)
            ...
            file_path_main = os.path.join(project_id, extrmaincon_output_dir_prefix + "_" + project_id, "refilter_mainconfig_spanned_results.tsv")
            ...
            for map_results_folder in map_results_folders:
                if os.path.isdir(map_results_folder):
                    output_bam = os.path.join(map_results_folder, "spanning_reads_sorted.bam")
                    folder_refilter = f"{map_results_folder}_{refilter_id}"
                    if os.path.exists(folder_refilter):
                        # 如果路径是一个文件，删除该文件
                        if os.path.isfile(folder_refilter):
                    ...
                    run_command(["python", os.path.join(dir_path, "check_mainconfig_spanning_reads.py"), "-s", output_sam, "-i", fasta_file, "-o", folder_refilter, "-cl", str(check_spanning_length), "-tl", str(extrmaincon_trimmed_length_reset), "-a", seqdepth_alignment_software])
                    
                    # 复制 .fasta 文件到目标目录,   # check_subconfig_spanning_reads.py 将其删除了
                    for fasta_file in glob.glob(os.path.join(map_results_folder, "*.fasta")):
                        shutil.copy(fasta_file, folder_refilter)
            ...
            if count == 1 and mode_old == "A":         # 当 spanning_length 相等时
                run_command(["python", os.path.join(dir_path, "filter_spanning_results.py"), "-sr", project_id + "/" + extrsubcon_output_dir_prefix + "_" + project_id + "/refilter_subconfig_spanned_results.tsv", "-mr", project_id + "/" + extrmaincon_output_dir_prefix + "_" + project_id + "/refilter_mainconfig_spanned_results.tsv", "-rf", project_id + "/" + f"ROUSFinder_results_{project_id}/" + rous_output_file_prefix + "_" + project_id + "_rep_5CT.tsv", "-rc", project_id + "/" + f"ROUSFinder_results_{project_id}/" + rous_output_file_prefix + "_" + project_id + "_rep_calibration_table.txt", "-o", output_dir_prefix_filter + "_" + refilter_id + "/", "-rn", str(read_number_again), "-cr", complementary_chain])
            elif count > 1 and mode_old == "A":
                run_command(["python", os.path.join(dir_path, "filter_spanning_results.py"), "-sr", project_id + "/" + extrsubcon_output_dir_prefix + "_" + project_id + "/refilter_subconfig_spanned_results.tsv", "-mr", project_id + "/" + extrmaincon_output_dir_prefix + "_" + project_id + "/refilter_mainconfig_spanned_results.tsv", "-rf", project_id + "/" + f"ROUSFinder_results_{project_id}/" + "pseudo_" + rous_output_file_prefix + "_" + project_id + "_rep_5CT.tsv", "-rc", project_id + "/" + f"ROUSFinder_results_{project_id}/" + "pseudo_" + rous_output_file_prefix + "_" + project_id + "_rep_calibration_table.txt", "-o", output_dir_prefix_filter + "_" + refilter_id + "/", "-rn", str(read_number_again), "-cr", complementary_chain])

            if count == 1 and mode_old == "C":         # 当 spanning_length 相等时
                run_command(["python", os.path.join(dir_path, "filter_spanning_results.py"), "-sr", project_id + "/" + extrsubcon_output_dir_prefix + "_" + project_id + "/refilter_subconfig_spanned_results.tsv", "-mr", project_id + "/" + extrmaincon_output_dir_prefix + "_" + project_id + "/refilter_mainconfig_spanned_results.tsv", "-rf", project_id + "/" + f"ROUSFinder_results_{project_id}/" + rous_output_file_prefix + "_" + project_id + "_rep_5CT.tsv", "-rc", project_id + "/" + manually_calibrate_reset, "-o", output_dir_prefix_filter + "_" + refilter_id + "/", "-rn", str(read_number_again), "-cr", complementary_chain])
            elif count > 1 and mode_old == "C":
                run_command(["python", os.path.join(dir_path, "filter_spanning_results.py"), "-sr", project_id + "/" + extrsubcon_output_dir_prefix + "_" + project_id + "/refilter_subconfig_spanned_results.tsv", "-mr", project_id + "/" + extrmaincon_output_dir_prefix + "_" + project_id + "/refilter_mainconfig_spanned_results.tsv", "-rf", project_id + "/" + f"ROUSFinder_results_{project_id}/" + "pseudo_" + rous_output_file_prefix + "_" + project_id + "_rep_5CT.tsv", "-rc", project_id + "/" + manually_calibrate_reset, "-o", output_dir_prefix_filter + "_" + refilter_id + "/", "-rn", str(read_number_again), "-cr", complementary_chain])
...
#########################################
        ...
        if mode == "N" and refilter_mode:
            ...
            logging.info(f"Please check the refiltered paired repeats in '{output_dir_prefix_filter}_{refilter_id}/paired_repeats_for_mapping.tsv' for mapping mitogenomic recombination.\n")
