...
def create_paired_data_sub(df):
    ...
    pair_lookup = {'AC': 'BD', 'BD': 'AC', 'AD': 'CB', 'CB': 'AD'}
        ...
        if group_df.shape[1] >= 4:
            ...
            for (split_3, split_4), sub_df in grouped:
                ...
                if matching_element is not None:
                    # Convert sorted_key to the desired format ('AC', 'BD')
...
def get_start_end(fragment_id, rep_5ct_file_path, rep_calibr_file_path):
    ...
    row_rep_calibr = rep_calibr_df[(rep_calibr_df['start'] == row_rep_5ct['start']) & (rep_calibr_df['end'] == row_rep_5ct['end'])].iloc[0]
        ...
        for main_group in main_paired_groups:
            if sub_group[0][2] == main_group[0][2] and sub_group[0][3] == main_group[0][3]:
                ...
                if unique_key not in unique_sub_rows:
                    ...
                    # Append to result_list
                    if sequence_type == "direct_repeat" and sub_group[0][1] == "AD":
                    ...
                    if sequence_type == "direct_repeat" and sub_group[0][1] == "CB":
                    ...
                    if sequence_type == "inverted_repeat" and sub_group[0][1] == "AC":
                    ...
                    if sequence_type == "inverted_repeat" and sub_group[0][1] == "BD":
...
def calculate_ratio(sub_group, main_group):
    # 初始化各个比例的变量
    ratio_DR_AD_C_AB_C_numerator, ratio_DR_AD_C_AB_C_denominator = 0, 0
    ratio_DR_AD_AB_numerator, ratio_DR_AD_AB_denominator = 0, 0
    ratio_DR_CB_C_CD_C_numerator, ratio_DR_CB_C_CD_C_denominator = 0, 0
    ratio_DR_CB_CD_numerator, ratio_DR_CB_CD_denominator = 0, 0
    ratio_IR_AC_CD_C_numerator, ratio_IR_AC_CD_C_denominator = 0, 0
    ratio_IR_AC_C_CD_numerator, ratio_IR_AC_C_CD_denominator = 0, 0
    ratio_IR_BD_C_AB_numerator, ratio_IR_BD_C_AB_denominator = 0, 0
    ratio_IR_BD_AB_C_numerator, ratio_IR_BD_AB_C_denominator = 0, 0
    ratio_DR_AD_AB = ratio_DR_AD_C_AB_C = ratio_DR_AD = ratio_DR_CB_CD = "N/A"
    ratio_DR_CB_C_CD_C = ratio_DR_CB = ratio_IR_AC_CD_C = ratio_IR_AC_C_CD = "N/A"
    ratio_IR_AC = ratio_IR_BD_AB_C = ratio_IR_BD_C_AB = ratio_IR_BD = "N/A"

    for sub_row in sub_group:
        if sub_row[1] == 'AD' and sub_row[4] == 'minus':
            ratio_DR_AD_C_AB_C_numerator = sub_row[7]
        if sub_row[1] == 'AD' and sub_row[4] == 'plus':
            ratio_DR_AD_AB_numerator = sub_row[7]
        if sub_row[1] == 'CB' and sub_row[4] == 'minus':
            ratio_DR_CB_C_CD_C_numerator = sub_row[7]
        if sub_row[1] == 'CB' and sub_row[4] == 'plus':
            ratio_DR_CB_CD_numerator = sub_row[7]
        if sub_row[1] == 'AC' and sub_row[4] == 'plus':
            ratio_IR_AC_CD_C_numerator = sub_row[7]
        if sub_row[1] == 'AC' and sub_row[4] == 'minus':
            ratio_IR_AC_C_CD_numerator = sub_row[7]
        if sub_row[1] == 'BD' and sub_row[4] == 'minus': 
            ratio_IR_BD_C_AB_numerator = sub_row[7]
        if sub_row[1] == 'BD' and sub_row[4] == 'plus':
            ratio_IR_BD_AB_C_numerator = sub_row[7]
            
    for main_row in main_group:
        if main_row[1] == 'AB' and main_row[4] == 'minus':
            ratio_DR_AD_C_AB_C_denominator = main_row[7]
        if main_row[1] == 'AB' and main_row[4] == 'plus':
            ratio_DR_AD_AB_denominator = main_row[7]
        if main_row[1] == 'CD' and main_row[4] == 'minus':
            ratio_DR_CB_C_CD_C_denominator = main_row[7]
        if main_row[1] == 'CD' and main_row[4] == 'plus':
            ratio_DR_CB_CD_denominator = main_row[7]
        if main_row[1] == 'CD' and main_row[4] == 'minus':
            ratio_IR_AC_CD_C_denominator = main_row[7]
        if main_row[1] == 'CD' and main_row[4] == 'plus':
            ratio_IR_AC_C_CD_denominator = main_row[7]
        if main_row[1] == 'AB' and main_row[4] == 'plus':
            ratio_IR_BD_C_AB_denominator = main_row[7]
        if main_row[1] == 'AB' and main_row[4] == 'minus':
            ratio_IR_BD_AB_C_denominator = main_row[7]
                
    # 计算综合性的 ratio
    if (ratio_DR_AD_C_AB_C_denominator + ratio_DR_AD_AB_denominator) > 0:
        ratio_DR_AD = (ratio_DR_AD_C_AB_C_numerator + ratio_DR_AD_AB_numerator) / (ratio_DR_AD_C_AB_C_denominator + ratio_DR_AD_AB_denominator)
        ratio_DR_AD = round(ratio_DR_AD, 6)
    else:
        ratio_DR_AD = 'N/A'
    ratio_DR_AD_AB = f"{ratio_DR_AD_AB_numerator}/{ratio_DR_AD_AB_denominator}"
    ratio_DR_AD_C_AB_C = f"{ratio_DR_AD_C_AB_C_numerator}/{ratio_DR_AD_C_AB_C_denominator}"

    if (ratio_DR_CB_C_CD_C_denominator + ratio_DR_CB_CD_denominator) > 0:
        ratio_DR_CB = (ratio_DR_CB_C_CD_C_numerator + ratio_DR_CB_CD_numerator) / (ratio_DR_CB_C_CD_C_denominator + ratio_DR_CB_CD_denominator)
        ratio_DR_CB = round(ratio_DR_CB, 6)
    else:
       ratio_DR_CB = 'N/A'
    ratio_DR_CB_CD = f"{ratio_DR_CB_CD_numerator}/{ratio_DR_CB_CD_denominator}"
    ratio_DR_CB_C_CD_C = f"{ratio_DR_CB_C_CD_C_numerator}/{ratio_DR_CB_C_CD_C_denominator}"

    if (ratio_IR_AC_CD_C_denominator + ratio_IR_AC_C_CD_denominator) > 0:
        ratio_IR_AC = (ratio_IR_AC_CD_C_numerator + ratio_IR_AC_C_CD_numerator) / (ratio_IR_AC_CD_C_denominator + ratio_IR_AC_C_CD_denominator)
        ratio_IR_AC = round(ratio_IR_AC, 6)
    else:
        ratio_IR_AC = 'N/A'
    ratio_IR_AC_CD_C = f"{ratio_IR_AC_CD_C_numerator}/{ratio_IR_AC_CD_C_denominator}"
    ratio_IR_AC_C_CD = f"{ratio_IR_AC_C_CD_numerator}/{ratio_IR_AC_C_CD_denominator}"


    if (ratio_IR_BD_C_AB_denominator + ratio_IR_BD_AB_C_denominator) > 0:
        ratio_IR_BD = (ratio_IR_BD_C_AB_numerator + ratio_IR_BD_AB_C_numerator) / (ratio_IR_BD_C_AB_denominator + ratio_IR_BD_AB_C_denominator)
        ratio_IR_BD = round(ratio_IR_BD, 6)
    else:
        ratio_IR_BD = 'N/A'
    ratio_IR_BD_AB_C = f"{ratio_IR_BD_AB_C_numerator}/{ratio_IR_BD_AB_C_denominator}"
    ratio_IR_BD_C_AB = f"{ratio_IR_BD_C_AB_numerator}/{ratio_IR_BD_C_AB_denominator}"
    
    return [ratio_DR_AD_AB, ratio_DR_AD_C_AB_C, ratio_DR_AD, ratio_DR_CB_CD, ratio_DR_CB_C_CD_C, ratio_DR_CB, ratio_IR_AC_CD_C, ratio_IR_AC_C_CD, ratio_IR_AC, ratio_IR_BD_AB_C, ratio_IR_BD_C_AB, ratio_IR_BD] 
