...
def trim_IR_sequence_circle(genome, fields, output_dir, trim_len, id1, id2, rev_compl_IR): 
    ...
    if len(genome_rev) - len(rep1_sequence) >= trim_len:
        ...
        if repeat1_rev:       #若重复单元在负链，则从负链截取
            bd_record = SeqRecord(bd_sequence, id=f"IR_RD_{id1}_{id2}_plus_{trim_len}", description="")              # 这里的正负链是以输入链为正链，重组后正链依然认为是正链
            SeqIO.write(bd_record, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if not repeat1_rev:   #若重复单元在正链，则直接保存
            bd_sequence = reverse_complement_dna(bd_sequence)
            bd_record = SeqRecord(bd_sequence, id=f"IR_RD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(bd_record, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat1_rev:  #考虑互补链，之前在负链上截取，则下面就再在正链上截取
            bd_sequence_c = reverse_complement_dna(bd_sequence)
            bd_record_c = SeqRecord(bd_sequence_c, id=f"IR_RD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(bd_record_c, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat1_rev:  #考虑互补链，之前在正链上截取，则下面就再在负链上截取
            bd_sequence_c = reverse_complement_dna(bd_sequence)
            bd_record_c = SeqRecord(bd_sequence_c, id=f"IR_RD_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(bd_record_c, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        ...
        if repeat1_rev:
            bd_record = SeqRecord(bd_sequence, id=f"IR_RD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(bd_record, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if not repeat1_rev:
            bd_sequence = reverse_complement_dna(bd_sequence)
            bd_record = SeqRecord(bd_sequence, id=f"IR_RD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(bd_record, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat1_rev:
            bd_sequence_c = reverse_complement_dna(bd_sequence) 
            bd_record_c = SeqRecord(bd_sequence_c, id=f"IR_RD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(bd_record_c, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat1_rev:
            bd_sequence_c = reverse_complement_dna(bd_sequence)
            bd_record_c = SeqRecord(bd_sequence_c, id=f"IR_RD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(bd_record_c, os.path.join(output_dir, f"IR_RD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        
        print(f"ATTENTION: The remaining length of the subconfiguration1 DR_RD_{id1}_{id2} minus the length of repeat {id1} is {lasso_trim_len} bp, which is shorter than that of the trimmed sequence {trim_len} bp. It is here reset the maximum allowable length of the trimmed sequence to {lasso_trim_len} bp automatically.") 
...
################################################################################
    if len(genome.seq) - len(rep2_sequence) >= trim_len:
        ...
        if repeat2_rev:
            ac_record = SeqRecord(ac_sequence, id=f"IR_LU_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ac_record, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if not repeat2_rev:
            ac_sequence = reverse_complement_dna(ac_sequence)
            ac_record = SeqRecord(ac_sequence, id=f"IR_LU_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ac_record, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat2_rev:
            ac_sequence_c = reverse_complement_dna(ac_sequence)
            ac_record_c = SeqRecord(ac_sequence_c, id=f"IR_LU_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ac_record_c, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat2_rev:
            ac_sequence_c = reverse_complement_dna(ac_sequence)
            ac_record_c = SeqRecord(ac_sequence_c, id=f"IR_LU_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ac_record_c, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
    else:
        ...
        if repeat2_rev:
            ac_record = SeqRecord(ac_sequence, id=f"IR_LU_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ac_record, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if not repeat2_rev:
            ac_sequence = reverse_complement_dna(ac_sequence)
            ac_record = SeqRecord(ac_sequence, id=f"IR_LU_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ac_record, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and repeat2_rev:
            ac_sequence_c = reverse_complement_dna(ac_sequence) 
            ac_record_c = SeqRecord(ac_sequence_c, id=f"IR_LU_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ac_record_c, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_IR and not repeat2_rev:
            ac_sequence_c = reverse_complement_dna(ac_sequence) 
            ac_record_c = SeqRecord(ac_sequence_c, id=f"IR_LU_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ac_record_c, os.path.join(output_dir, f"IR_LU_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        
        print(f"ATTENTION: The remaining length of the subconfiguration2 DR_LU_{id1}_{id2} minus the length of repeat {id2} is {lasso_trim_len} bp, which is shorter than that of the trimmed sequence {trim_len} bp. It is here reset the maximum allowable length of the trimmed sequence  to {lasso_trim_len} bp automatically.") 
...
def trim_IR_sequence_linear(genome, fields, output_dir, trim_len, id1, id2, rev_compl_IR): 
    ...
    if upstream_length < trim_len or downstream_length < trim_len:
        ...
        print(f"ATTENTION: The length of one of the sequences on the left and right sides of {id1} in the subconfiguration1 IR_RD_{id1}_{id2} is less than that of the trimmed sequence length ({trim_len} bp). It is here reset the maximum allowable length of the trimmed sequence  to {lasso_trim_len} bp automatically.") 
...
################################################################################
    ...
    if upstream_length < trim_len or downstream_length < trim_len:
        ...
        print(f"ATTENTION: The length of one of the sequences on the left and right sides of {id2} in the subconfiguration2 IR_LU_{id1}_{id2} is less than that of the trimmed sequence length ({trim_len} bp). It is here reset the maximum allowable length of the trimmed sequence  to {lasso_trim_len} bp automatically.") 
...
def trim_DR_sequence_circle(genome, fields, output_dir, trim_len, id1, id2, rev_compl_DR):
    ...
    if len(genome_sub1) - len(rep1_sequence) >= trim_len:
        ...
        if repeat1_rev:
            ...
            ad_record = SeqRecord(ad_sequence, id=f"DR_LD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ad_record, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if not repeat1_rev:
            ad_record = SeqRecord(ad_sequence, id=f"DR_LD_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ad_record, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat1_rev:
            ad_sequence_c = reverse_complement_dna(ad_sequence)
            ad_record_c = SeqRecord(ad_sequence_c, id=f"DR_LD_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(ad_record_c, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat1_rev:
            ad_sequence_c = reverse_complement_dna(ad_sequence)
            ad_record_c = SeqRecord(ad_sequence_c, id=f"DR_LD_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(ad_record_c, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
    else:
        ...
        if repeat1_rev:
            ad_sequence = reverse_complement_dna(ad_sequence)
            ad_record = SeqRecord(ad_sequence, id=f"DR_LD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ad_record, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if not repeat1_rev:
            ad_record = SeqRecord(ad_sequence, id=f"DR_LD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ad_record, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat1_rev:
            ad_sequence_c = reverse_complement_dna(ad_sequence)
            ad_record_c = SeqRecord(ad_sequence_c, id=f"DR_LD_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(ad_record_c, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat1_rev:
            ad_sequence_c = reverse_complement_dna(ad_sequence)
            ad_record_c = SeqRecord(ad_sequence_c, id=f"DR_LD_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(ad_record_c, os.path.join(output_dir, f"DR_LD_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")

        print(f"ATTENTION: The remaining length of the subconfiguration1 DR_LD_{id1}_{id2} minus the length of repeat {id1} is {lasso_trim_len} bp, which is shorter than that of the trimmed sequence {trim_len} bp. It is here reset the maximum allowable length of the trimmed sequence  to {lasso_trim_len} bp automatically.") 
...
################################################################################
    if len(genome_sub2) - len(rep2_sequence) >= trim_len:
        ...
        if repeat2_rev:
            ...
            cb_record = SeqRecord(cb_sequence, id=f"DR_UR_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(cb_record, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
        if not repeat2_rev:
            cb_record = SeqRecord(cb_sequence, id=f"DR_UR_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(cb_record, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat2_rev:
            cb_sequence_c = reverse_complement_dna(cb_sequence)
            cb_record_c = SeqRecord(cb_sequence_c, id=f"DR_UR_{id1}_{id2}_plus_{trim_len}", description="")
            SeqIO.write(cb_record_c, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_plus_{trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat2_rev:
            cb_sequence_c = reverse_complement_dna(cb_sequence)
            cb_record_c = SeqRecord(cb_sequence_c, id=f"DR_UR_{id1}_{id2}_minus_{trim_len}", description="")
            SeqIO.write(cb_record_c, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_minus_{trim_len}.fasta"), "fasta")
    else:
        # 创建UR
        ...
        if repeat2_rev:
            cb_sequence = reverse_complement_dna(cb_sequence)
            cb_record = SeqRecord(cb_sequence, id=f"DR_UR_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(cb_record, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")
        if not repeat2_rev:
            cb_record = SeqRecord(cb_sequence, id=f"DR_UR_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(cb_record, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and repeat2_rev:
            cb_sequence_c = reverse_complement_dna(cb_sequence)
            cb_record_c = SeqRecord(cb_sequence_c, id=f"DR_UR_{id1}_{id2}_plus_{lasso_trim_len}", description="")
            SeqIO.write(cb_record_c, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_plus_{lasso_trim_len}.fasta"), "fasta")
        if rev_compl_DR and not repeat2_rev:
            cb_sequence_c = reverse_complement_dna(cb_sequence)
            cb_record_c = SeqRecord(cb_sequence_c, id=f"DR_UR_{id1}_{id2}_minus_{lasso_trim_len}", description="")
            SeqIO.write(cb_record_c, os.path.join(output_dir, f"DR_UR_{id1}_{id2}_minus_{lasso_trim_len}.fasta"), "fasta")

        print(f"ATTENTION: The remaining length of the subconfiguration2 DR_UR_{id1}_{id2} minus the length of repeat {id2} is {lasso_trim_len} bp, which is shorter than that of the trimmed sequence {trim_len} bp. It is here reset the maximum allowable length of the trimmed sequence  to {lasso_trim_len} bp automatically.") 
...
def trim_DR_sequence_linear(genome, fields, output_dir, trim_len, id1, id2, rev_compl_DR):
    ...
    if upstream_length < trim_len or downstream_length < trim_len:
        ...
        print(f"ATTENTION: The length of one of the sequences on the left and right sides of {id1} in the subconfiguration1 IR_LD_{id1}_{id2} is less than that of the trimmed sequence length ({trim_len} bp). It is here reset the maximum allowable length of the trimmed sequence  to {lasso_trim_len} bp automatically.") 
...
################################################################################
    ...
    if upstream_length < trim_len or downstream_length < trim_len:
        ...
        print(f"ATTENTION: The length of one of the sequences on the left and right sides of {id2} in the subconfiguration2 IR_UR_{id1}_{id2} is less than that of the trimmed sequence length ({trim_len} bp). It is here reset the maximum allowable length of the trimmed sequence  to {lasso_trim_len} bp automatically.") 
...
def find_all_direct_repeat_pairs(new_data):
    ...
    while error_count <= 3:
        try:
            ...
            if sorted_pairs_count != original_pairs_count / 2:
                print(f"ERROR: Not all repetitive sequences have two repeat units. Check {direct_repeat_pairs}.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(columns) >= 5:  # 假设 start 和 end 是第 2 和第 3 列（基于从0开始的索引）
            ...
            if not max_value == genome_length:
                print(f"ERROR: The genome length is {genome_length}, which is different the length ({max_value}) in the five-colume table.")
                sys.exit(1)
    except OSError as e:
    ...
    with open(args.repeattable, 'r', encoding='utf-8') as file:
