...
def update_inputfasta(data_5CT, id1, id2, inputfasta, output_file, genome_type): 
    ...
    if genome_type == "Circle":
        with open(output_file_1, 'w', encoding='utf-8') as fasta_file:  
            fasta_header = f">DR_{id1}_{repeat1_start}_{repeat1_end}_{id2}_{repeat2_start}_{repeat2_end}_chr1_{len(genome_sub1)}_{genome_type}_1to2"  
        ...
        with open(output_file_2, 'w', encoding='utf-8') as fasta_file:  
            fasta_header = f">DR_{id1}_{repeat1_start}_{repeat1_end}_{id2}_{repeat2_start}_{repeat2_end}_chr2_{len(genome_sub2)}_{genome_type}_1to2"  
            fasta_file.write(f"{fasta_header}\n")  
        ...
        with open(output_file_2, 'w', encoding='utf-8') as fasta_file:  
            fasta_header = f">DR_{id1}_{repeat1_start}_{repeat1_end}_{id2}_{repeat2_start}_{repeat2_end}_chr2_{len(genome_sub2)}_circle_1to2"  
            fasta_file.write(f"{fasta_header}\n")  
            for i in range(0, len(genome_sub2), 100):  
                fasta_file.write(str(genome_sub2[i : i+100]) + "\n") 
