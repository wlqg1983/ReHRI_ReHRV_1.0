...
def validate_manual_calibrated_list(manual_calibrated_list, count):
    ...
    if num_columns != 6:
        ...
        print("The correct header should be: fragment_id, length, start, end, direction, chromosome. (TSV format)\n")
        exit(1)
...
def validate_chromosome_headers(fasta_file, chromosomes):
    ...
    if sorted(fasta_headers) != sorted(chromosomes):
        print("Warning: Each chromosome will be labeled by chr1, chr2, ... in turn according to the order of chromosomes in the {fasta_file}.\n")
        time.sleep(10)  # 等待用户终止程序
